    ...
    class ReduxFramework_slides {
        ...
        public function render () {
            ...
            $defaults = array(
                ...
                'content_title' => esc_html__( 'Slide', 'hoskia' )
            );

            $this->field = wp_parse_args ( $this->field, $defaults );

            echo '<div class="redux-slides-accordion" data-new-content-title="' . esc_attr ( sprintf ( esc_html__( 'New %s', 'hoskia' ), $this->field[ 'content_title' ] ) ) . '">';
            ...
            if ( isset ( $this->value ) && is_array ( $this->value ) && !empty ( $this->value ) ) {
                ...
                foreach ( $slides as $slide ) {
                    ...
                    echo '<img class="redux-slides-image" id="image_image_id_' . $x . '" src="' . $slide[ 'thumb' ] . '" alt="'.esc_attr__( 'Image', 'hoskia' ).'" target="_blank" rel="external" />';
                    echo '</a>';
                    echo '</div>';
                    ...
                    echo '<span class="button media_upload_button" id="add_' . $x . '">' . esc_html__( 'Upload', 'hoskia' ) . '</span>';
                    ...
                    echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $slide[ 'attachment_id' ] . '">' . esc_html__( 'Remove', 'hoskia' ) . '</span>';
                    ...
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'title' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'title' ] ) : esc_html__( 'Title', 'hoskia' );
                    ...
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'progress' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'progress' ] ) : esc_html__( 'Progress', 'hoskia' );
					...
					// font awesome icon
					$icons = hoskia_fa_icons();
					$showicon = '';
					foreach( $icons as $key=>$value ){
						$showicon .= '<span class="seticon" data-icon="'.$key.'">'.$value.'</span>';
					}

                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'icon' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'icon' ] ) : esc_html__( 'Icon', 'hoskia' );
					
                    echo '<li '.$disnone.'><span class="showicon">'.esc_html__( 'Show Icon', 'hoskia' ).'</span>
					<div id="iconbox" class="iconbox"><input id="searchfield" placeholder="'.esc_attr__( 'Search icon...', 'hoskia' ).'" class="searchfield"  type="search" size="20" maxlength="30">'.$showicon.'</div>
                    ...
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'facts-number' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'facts-number' ] ) : esc_html__( 'Facts Number', 'hoskia' );
                    ...
                    if ( $this->field[ 'show' ][ 'description' ] ) {
                        $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'description' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'description' ] ) : esc_html__( 'Description', 'hoskia' );
                    ...
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'url' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'url' ] ) : esc_html__( 'URL', 'hoskia' );
                    ...
                    echo '<li><a href="javascript:void(0);" class="button deletion redux-slides-remove">' . esc_html__( 'Delete', 'hoskia' ) . '</a></li>';
            ...
            if ( $x == 0 ) {
                echo '<div class="redux-slides-accordion-group"><fieldset class="redux-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="redux-slides-header">' . esc_attr ( sprintf ( esc_html__( 'New %s', 'hoskia' ), $this->field[ 'content_title' ] ) ) . '</span></h3><div>';

                $hide = ' hide';

                echo '<div class="screenshot' . $hide . '">';
                echo '<a class="of-uploaded-image" href="">';
                echo '<img class="redux-slides-image" id="image_image_id_' . $x . '" src="" alt="'.esc_attr__( 'Image', 'hoskia' ).'" target="_blank" rel="external" />';
                echo '</a>';
                echo '</div>';
                ...
                echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $this->parent->args[ 'opt_name' ] . '[' . $this->field[ 'id' ] . '][attachment_id]">' . esc_html__( 'Remove', 'hoskia' ) . '</span>';
                ...
                echo '<li '.$disnone.'><span class="showicon">'.esc_html__( 'Show Icon', 'hoskia' ).'</span>
					<div id="iconbox" class="iconbox"><input id="searchfield" class="searchfield"  type="search" placeholder="'.esc_attr__( 'Search icon...', 'hoskia' ).'" size="20" maxlength="30">'.$showicon.'</div>
            ...
            echo '</div><a href="javascript:void(0);" class="button redux-slides-add button-primary" rel-id="' . $this->field[ 'id' ] . '-ul" rel-name="' . $this->field[ 'name' ] . '[title][]' . $this->field['name_suffix'] .'">' . sprintf ( esc_html__( 'Add %s', 'hoskia' ), $this->field[ 'content_title' ] ) . '</a><br/>';
        }

<?php 
/**
 * Quick Fix Section Heading Element
 */
class WPBakeryShortCode_team_slider extends WPBakeryShortCode {
	...
	function __construct(){
		...
		add_action( 'vc_build_admin_page' , array( $this, 'hoskia_teamslider_maping' ) );
		
		// Qfix feature shortcode
		add_shortcode( 'ssdhostteamslider', array( $this, 'hoskia_teamslider_shortcode' ) );
		
	}
	
	// vc Param
	public function hoskia_teamslider_maping(){
		...
		vc_map( array(
		  "name" => esc_html__( "Team Members Slider", "hoskia" ),
		  "base" => "ssdhostteamslider",
		  "class" => "",
		  ...
		  "params" => array(
			...
			array(
				"type" => "param_group",
				'param_name' => 'members',
				'heading' => esc_html__('Set member information', "hoskia" ),
				"group"		=> esc_html__( "Member Info Set", "hoskia" ),
				'params' => array(
					array(
						"type" => "attach_image",
						"holder" => "div",
			...
			array(
				"type" 		 => "floatnumber",
				"holder" 	 => "div",
				"heading" 	 => esc_html__( "Per row Slide Number", "hoskia" ),
	...
	public function hoskia_teamslider_shortcode( $atts, $content = null ){
		...
		$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, vc_shortcode_custom_css_class( $css, ' ' ), 'ssdhostteamslider', $atts );
		...
		?>
		<!-- Team Members Start -->
		<div class="team--members owl-carousel owl--dots" data-carousel-items="4" data-carousel-autoplay="true" data-carousel-margin="30" data-carousel-nav="true" data-carousel-hover-pause="true" data-carousel-responsive='{"0": {"items": "1"}, "480": {"items": "2"}, "768": {"items": "3"}, "992": {"items": "<?php echo esc_attr( $slidenumber ); ?>"}}'>
			...
			foreach( $members as $member ):
				...
				$imgurl = '';
				if( !empty( $member['img'] ) ){
					$imgurl = wp_get_attachment_image_src( $member['img'] , 'full' );
				}
				...
				<div class="team--member-img">
					...
					<div class="team--member-info bg--overlay">
						...
						if( !empty( $member['name'] ) ){
							echo '<h3 class="h4">'.esc_html( $member['name'] ).'</h3>';
						}
						//
						if( !empty( $member['expertin'] ) ){
							echo '<p>'.esc_html( $member['expertin'] ).'</p>';
						}
						
						$teamsocial = '';
						if( !empty( $member['teamsocial'] ) ){
							$teamsocial = vc_param_group_parse_atts( $member['teamsocial'] );
						}
...
$sectheading = new WPBakeryShortCode_team_slider();

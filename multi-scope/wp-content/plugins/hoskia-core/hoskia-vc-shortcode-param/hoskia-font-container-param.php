 ...
 *                'color',
 *
 *                'tag_description' => __('Select element tag.','hoskia'),
 *                'text_align_description' => __('Select text alignment.','hoskia'),
 *                'font_size_description' => __('Enter font size.','hoskia'),
 *                'line_height_description' => __('Enter line height.','hoskia'),
 *                'color_description' => __('Select color for your element.','hoskia'),
 *            ),
 *        ),
 *    ),
...
class HOSKIA_Font_Container {
	...
	function __construct() {
		if ( function_exists( 'vc_add_shortcode_param' ) ) {
			vc_add_shortcode_param( 'hoskia_font_container', array( $this, 'vc_font_container_form_field' ),  plugins_url( '/js/hoskia_additional_param.js', __FILE__ ) );
		}
	...
	function vc_font_container_form_field( $settings, $value ) {
		$font_container = new HOSKIA_Font_Container();
	...
	public function render( $settings, $value ) {
		$fields = array();
		$values = array();
		extract( $this->_hoskia_font_container_parse_attributes( $settings['settings']['fields'], $value ) );

		$data   = array();
		$output = '<div class="vc_row hoskia_vc">';
		if ( ! empty( $fields ) ) {
			...
			if ( isset( $fields['tag'] ) ) {
                ...
                <div class="vc_col-' . $columns . ' ">
                    <div class="wpb_element_label">' . __( 'Element tag', 'hoskia' ) . '</div>
				...
				$tags        = $this->_hoskia_font_container_get_allowed_tags();
				foreach ( $tags as $tag ) {
			...
			if ( isset( $fields['font_size'] ) ) {
				$data['font_size'] = '
                <div class="vc_col-' . $columns . '">
                    <div class="wpb_element_label">' . __( 'Font size', 'hoskia' ) . '</div>
                    <div class="crum-number-field-wrap vc_font_container_form_field-font_size-container">
                    <input type="number" min="0" step="1" class="Virsky_number_field wpb_vc_param_value vc_font_container_form_field-font_size-input" value="' . $values['font_size'] . '" />
                    </div>';
			...
			if ( isset( $fields['line_height'] ) ) {
				$data['line_height'] = '
                <div class="vc_col-' . $columns . '">
                    <div class="wpb_element_label">' . __( 'Line height', 'hoskia' ) . '</div>
                    <div class="crum-number-field-wrap vc_font_container_form_field-line_height-container">
                        <input type="number" step="1" min="0" class="Virsky_number_field vc_font_container_form_field-line_height-input wpb_vc_param_value"  value="' . $values['line_height'] . '" />
                    </div>';
			...
			if ( isset( $fields['color'] ) ) {
				$data['color'] = '
                <div class="vc_col-' . $columns . '">
                    <div class="wpb_element_label">' . __( 'Text color', 'hoskia' ) . '</div>
			...
			if ( isset( $fields['letter_spacing'] ) ) {
				$data['letter_spacing'] = '
                <div class="vc_col-' . $columns . '">
                    <div class="wpb_element_label">' . __( 'Letter spacing', 'hoskia' ) . '</div>
                    <div class="crum-number-field-wrap vc_font_container_form_field-letter_spacing-container">
                        <input type="number" min="0" step="1" class="Virsky_number_field vc_font_container_form_field-letter_spacing-input wpb_vc_param_value" value="' . $values['letter_spacing'] . '" />
                    </div>';
			...
			if ( isset( $fields['font_style'] ) ) {
				...
				$data['font_style'] .= '<div class="wpb_element_label">' . __( 'Font style', 'hoskia' ) . '</div>';
				...
				if(isset($values['font_style_italic'])){
					...
					$data['font_style'] .= '<span class="vc_font_container_form_field-font_style-label italic"> ' . esc_html__( 'Italic', 'hoskia' ) . '</span>';
					$data['font_style'] .= '</label>';
				}
				...
				if(isset($values['font_style_underline'])){
					...
					$data['font_style'] .= '<span class="vc_font_container_form_field-font_style-label underline"> ' . esc_html__( 'Underline', 'hoskia' ) . '</span>';
					$data['font_style'] .= '</label>';
				}
				...
				if(isset($values['font_style_bold'])){
					...
					$data['font_style'] .= '<span class="vc_font_container_form_field-font_style-label bold"> ' . esc_html__( 'Bold', 'hoskia' ) . '</span>';
					$data['font_style'] .= '</label>';
				}
	 ...
	 */
	public function _hoskia_font_container_get_allowed_tags() {
		$allowed_tags = array(
			'h1',
			'h2',
			'h3',
		...
		return apply_filters( 'hoskia_font_container_get_allowed_tags', $allowed_tags );
	 ...
	 * @return array
	 */
	public function _hoskia_font_container_parse_attributes( $attr, $value ) {
		$fields = array();
		if ( isset( $attr ) ) {
...
if ( class_exists( 'HOSKIA_Font_Container' ) ) {
	$hoskia_font_container = new HOSKIA_Font_Container();
}

...
class Voip_Posttype{
    ...
    public function __construct( $args = array() ){
        ...
        $this->post_type_name = self::voip_postTypeName_prepare( $args['post_type'] );
        ...
        if( !post_type_exists( $this->post_type_name ) ){
            ...
            add_action( 'init', array( $this, 'voip_register_post_type' ) );
            
        }
       
    }
    // Post type register
    public function voip_register_post_type(){
        ...
        $defaultargs = array(
            ...
            'show_in_menu'       => true,
            'query_var'          => true,
            'rewrite'            => array( 'slug' => $this->voip_slug_prepare( $this->reg_type_name ) ),
    ...
    // register taxonomy 
    public function voip_register_taxonomy( $args = array() ){
        ...
        if( !empty( $args['taxname'] ) ){
            ...
            $taxName      = self::voip_postTypeName_prepare( $args['taxname'] );
        }
        ...
        $defaultArgs = array(
            ...
            'query_var'         => true,
            'rewrite'           => array( 'slug' => $this->voip_slug_prepare( $taxName ) ),
        );
    ...
    private static function voip_postTypeName_prepare( $name ){
    ...
    // Label Plural name
    private static function voip_pluralName( $name ){
        
        $name = self::voip_namePrepare( $name );
    ...
    // label name prepare
    private static function voip_namePrepare( $name ){
    ...
    // slug prepare
    private function voip_slug_prepare( $name ){
